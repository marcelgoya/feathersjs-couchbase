      ...
      this.schema()
        .then(bucket => bucket
          .query(this._buildN1QL(couchbase, queryStr, consistency, true), values, (err, results, queryData) => {
            if(this.debug) {
              console.log(`CB QUERY RESULTS: ${$queryData.metrics.executionTime}`);
            }
    ...
    return new Promise((resolve, reject) => {
      this.get(id, params)
        .then(_obj => this.schema()
          .then(bucket => bucket
            .replace(this._key(id), Object.assign(Object.assign(_obj, data), { type: this.opts.name }), (err, result) => {
